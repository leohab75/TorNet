...
if [[ -n $(grep -i "ubuntu" /etc/os-release) ]]; then
    ...
    apt install zenity wget openresolv net-tools openvpn curl -y
    ...
    dnf install tor zenity wget curl openresolv net-tools openvpn torsocks pip -y
...
#xterm color as root
cp /tmp/TorNet/source/.Xresources /root/
...
echo -e "\n$GREEN*$BLUE настройка redsocks\n $RESETCOLOR"

rm -f /etc/redsocks.conf
mv /tmp/TorNet/source/redsocks.conf /etc/redsocks.conf
...
chmod +x /usr/local/bin/TorNet/bin/redsocks2
